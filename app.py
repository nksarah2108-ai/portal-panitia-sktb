...
# --- FUNGSI FIX LINK GOOGLE DRIVE (VERSI LEBIH STABIL) ---
def fix_drive_url(url):
    if "drive.google.com" in url:
        try:
            # Mengambil ID fail dari pelbagai format URL Drive
            if "/file/d/" in url:
                ...
                return url
            # Gunakan format uc?export=view yang paling stabil untuk Streamlit
...
st.markdown("""
    ...
    /* GAYA GAMBAR KETUA PANITIA (FIT PENUH) */
    .head-img-container {
        display: flex;
    ...
    .head-img {
        ...
        /* PENTING: object-fit cover paksa gambar penuhkan ruang */
        object-fit: cover;
        object-position: top center;
        display: block;
        box-shadow: 0 4px 15px rgba(0,0,0,0.3);
        background-color: white; /* Elak nampak kosong jika loading lambat */
    }

    /* SIDEBAR STYLE */
    [data-testid="stSidebar"] { background-color: #fce4ec !important; border-right: 2px solid #f8bbd0; }
    [data-testid="stSidebar"] p, div[role="radiogroup"] label p {
...
# --- 4. LAMAN PANITIA ---
    ...
    with h_col1:
        if KP_IMAGE_URL:
            # Gunakan img tag HTML secara terus untuk kawalan CSS penuh
    ...
    # (Bahagian Fail B, C, D dikekalkan seperti struktur anda sebelum ini...)
    with col2:
